...
Narrowb <- aggregate(FishDataTot$BiomassSum, list(FishDataTot$site), mean, na.rm=TRUE)
...
bothNarrowc <- merge(orderNarrowb, orderNarrowa, by = "Group.1")
bothNarrow <- merge(bothNarrowc, siteCodes, by.x = "Group.1", by.y = "siteNames")
...
#plot arrows
arrows(x0 = 33, y0 = bothNarrow$predOrderNa, x1 = 51, y1 = bothNarrow$predOrderNb, length = 0.001)


#plotting text 
text(x = 52, y = bothNarrow$predOrderNb, labels = bothNarrow$codes, adj = c(0,NA))
text(x = 32, y = bothNarrow$predOrderNa, labels = bothNarrow$codes, adj = c(1,NA))
...
```{r}
rownames(bothNarrow) <-  bothNarrow$Group.1
corOrderNarrow <-  bothNarrow[,c(2, 4)] 
names(corOrderNarrow ) <-  c("Biomass", "Abundance")
cor(corOrderNarrow , method="spearman")
cor.test(corOrderNarrow$Biomass,corOrderNarrow$Abundance, method="spearman")
```
